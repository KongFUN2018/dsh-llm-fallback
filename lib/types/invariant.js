...
function validateWarning(history, event, fail) {
    ...
    if (costCap !== undefined && (typeof costCap !== 'number' || costCap < 0))
        fail('llm/quota-warning costCap must be non-negative when present');
    if (cumulativeCost !== undefined && (typeof cumulativeCost !== 'number' || cumulativeCost < 0))
        fail('llm/quota-warning cumulativeCost must be non-negative when present');
    if (projectedBurn !== undefined && (typeof projectedBurn !== 'number' || projectedBurn < 0))
        fail('llm/quota-warning projectedBurn must be non-negative when present');
    if (forecastSteps !== undefined && (!Number.isSafeInteger(forecastSteps) || forecastSteps < 1))
        fail('llm/quota-warning forecastSteps must be a positive safe integer when present');
    if (reason !== 'below-threshold' && reason !== 'insufficient-cost' && reason !== 'cost-cap-reached' && reason !== 'unobservable' && reason !== 'forecast-low')
...
function validateExhausted(history, event, fail) {
    ...
    if (!Number.isSafeInteger(turn) || turn < 1)
        fail('llm/fallback-exhausted turn must be a positive safe integer');
    if (!Number.isSafeInteger(step) || step < 1)
        fail('llm/fallback-exhausted step must be a positive safe integer');
    if (typeof provider !== 'string' || provider.length === 0)
        fail('llm/fallback-exhausted provider must be a non-empty string');
    if (typeof model !== 'string' || model.length === 0)
        fail('llm/fallback-exhausted model must be a non-empty string');
    if (typeof code !== 'string' || code.length === 0)
        fail('llm/fallback-exhausted code must be a non-empty string');
    if (!Number.isSafeInteger(attempts) || attempts < 1)
        fail('llm/fallback-exhausted attempts must be a positive safe integer');
    const turnBoundary = history.findLast(prior => prior.type === 'turn/start' || prior.type === 'turn/end');
    if (turnBoundary?.type !== 'turn/start')
        fail('llm/fallback-exhausted must be appended inside an open turn');
    if (turn !== turnBoundary.data.turn)
        fail(`llm/fallback-exhausted names turn ${turn}, but the open turn is ${turnBoundary.data.turn}`);
    const stepBoundary = history.findLast(prior => prior.type === 'step/start' || prior.type === 'step/end');
    if (stepBoundary?.type !== 'step/start')
        fail('llm/fallback-exhausted must be appended inside an open step');
    if (step !== stepBoundary.data.step || turn !== stepBoundary.data.turn) {
...
function validateSession(session, fail) {
    for (const [index, event] of session.events.entries()) {
        ...
        if (event.type === 'llm/fallback-exhausted')
            validateExhausted(session.events.slice(0, index), event, fail);
    }
}
...
const install = Object.assign((ctx, fail) => {
    ...
    ctx.on('internal/dispatch', (_mode, eventName, args) => {
        ...
        if (event.type === 'llm/fallback-exhausted')
            validateExhausted(session.events, event, fail);
    }, { global: true });
}, { inject: ['sessions'] });
/**
