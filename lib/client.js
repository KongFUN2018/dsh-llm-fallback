	...
	factory: (require) => {
		...
		*/
		...
		function warningOf(event) {
			...
			if (data.reason !== "below-threshold" && data.reason !== "insufficient-cost" && data.reason !== "unobservable") return void 0;
			const remaining = count(data.remaining);
		...
		*/
		...
		function QuotaWarningNodeView({ node }) {
			...
			const main = data.reason === "unobservable" ? fbT("warning.unobservableProbe", { route: data.route }) : data.reason === "insufficient-cost" && remaining !== void 0 ? fbT("warning.insufficientCost", {
				route: data.route,
