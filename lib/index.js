...
async function capabilityOf(ctx, route) {
	const info = await ctx.llm.resolveModelInfo(route.provider, route.model).catch(() => void 0);
	return {
...
function apply(ctx, config = { fallbacks: [] }) {
	...
	ctx.on("agent/request", async (payload, next) => {
		...
		const userSwitched = isFreshPrimary && previousPrimary !== void 0 && (previousPrimary.provider !== resolved.provider || previousPrimary.model !== resolved.model);
