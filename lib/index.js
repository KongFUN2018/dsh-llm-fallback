...
function selectByStrategy(candidates, settings, inputTokens, requiredModalities, allowUnknownCapacity) {
	...
	const pool = candidates.filter((candidate) => passesFloor(candidate, floor, requiredModalities, allowUnknownCapacity));
...
function matchModel(primary, candidates, opts) {
	...
	let group;
	if (target === void 0) group = nonDegrading.length > 0 ? nonDegrading : opts.allowUnknownCapacity ? unknown : [];
	else group = nonDegrading.length > 0 ? nonDegrading : opts.allowDegrade ? degrading : opts.allowUnknownCapacity ? unknown : [];
...
async function capabilityOf(ctx, route) {
	...
	return {
		...
		...info.inputModalities === void 0 ? {} : { modalities: info.inputModalities }
	};
}
...
async function selectModel(ctx, primary, provider, opts) {
	...
	for (const model of models) {
		...
		candidates.push({
			...
			...info.reasoning === void 0 ? {} : { hasReasoning: true }
		});
	}
...
async function selectNextByDecision(ctx, chain, cursor, primary, banned, now, decisionProvider) {
	...
	for (let index = cursor; index < chain.length; index++) {
		...
		for (const id of ids) {
			...
			const capability = await capabilityOf(ctx, {
				provider: entry.provider,
				model: id
			});
			candidates.push({
				provider: entry.provider,
				model: id,
				...capability.contextWindow === void 0 ? {} : { contextWindow: capability.contextWindow },
				...capability.modalities === void 0 ? {} : { modalities: [...capability.modalities] }
			});
			routes.push({
				provider: entry.provider,
				model: id,
	...
	if (routes.length === 0) return void 0;
	const input = {
		primary: {
			...primary.contextWindow === void 0 ? {} : { contextWindow: primary.contextWindow },
			...primary.modalities === void 0 ? {} : { modalities: [...primary.modalities] }
		},
		candidates
	};
	const decided = await decisionProvider.decide(input).catch(() => void 0);
	if (decided === void 0) return void 0;
...
function apply(ctx, config = { fallbacks: [] }) {
	...
	const checkQuota = async (provider, model, signal) => {
		...
		const task = (async () => {
			for (const source of quota?.providers ?? []) {
				const result = await source.check(provider, model, signal).catch(() => void 0);
			...
			if (quota?.deepseek !== void 0 && provider === (quota.deepseek.provider ?? "deepseek-official")) {
	...
	ctx.on("agent/request", async (payload, next) => {
		...
		if (agentState.lastTurn !== void 0 && agentState.lastTurn !== turn) {
			...
			for (const key of [...agentState.steps.keys()]) if (key.startsWith(retired)) {
		...
		if (healthy !== void 0 && (healthy.provider !== resolved.provider || healthy.model !== resolved.model)) {
...
//#endregion
export { Config, DEFAULT_FALLBACK_CODES, DEFAULT_UNUSABLE_CODES, apply, buildFloor, comparePerformance, costScore, getFallbackStats, inject, name, passesFloor, priceOf, selectByStrategy };
