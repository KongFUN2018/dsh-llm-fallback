...
const Config = z.object({
	...
	probe: z.object({
		...
		timeoutMs: z.number().min(1).default(6e3),
...
async function selectNextByStrategy(catalog, chain, cursor, primary, opts, run, banned, now) {
	...
	for (let index = cursor; index < chain.length; index++) {
		...
		for (let i = 0; i < ids.length; i++) {
			...
			const projected = price === void 0 || price.input === void 0 && price.output === void 0 ? void 0 : (inputTokens * (price.input ?? 0) + outputTokens * (price.output ?? 0)) / 1e6;
			if (projected !== void 0) {
...
function createAgentTracker(ctx, clearQuota, onReset) {
	...
	const reset = () => {
		...
		for (const agent of [...knownAgents]) {
			...
			agentState.forecastWarnedRoute = void 0;
	...
	const stateFor = (agent) => {
		...
		if (state === void 0) {
			state = {
				...
				switchSuccess: 0,
				exhaustions: 0,
				forecastWarnedRoute: void 0,
				outputSamples: /* @__PURE__ */ new Map()
			};
...
function apply(ctx, config = { fallbacks: [] }) {
	...
	const strategyRun = (mode, session, failedRoutes, checkQuota, signal, agentState) => mode === void 0 || strategySettings === void 0 ? void 0 : {
		mode,
		settings: {
			...strategySettings,
			mode
	...
	const probeTimeoutMs = probe?.timeoutMs ?? 6e3;
	...
	*/
	async function probeValidRoute(route, signal) {
		if (ctx.llm === void 0) return { ok: true };
		...
		try {
			const linked = signal !== void 0 ? AbortSignal.any([signal, abort.signal]) : abort.signal;
			...
			let sawAny = false;
			for await (const chunk of stream) if (chunk.type === "block-start" || chunk.type === "text-delta" || chunk.type === "reasoning-delta" || chunk.type === "block-end") {
				sawAny = true;
				if (chunk.type === "block-start" || chunk.type === "text-delta" || chunk.type === "reasoning-delta") break;
			} else if (chunk.type === "finish") {
				if (chunk.reason.kind === "stop") return { ok: true };
	...
	* folded `usage.outputTokens` samples, or undefined before any sample. */
	const rollingOutputTokens = (agentState, provider) => {
		...
		if (samples === void 0 || samples.length === 0) return void 0;
		let sum = 0;
	...
	ctx.on("agent/request", async (payload, next) => {
		...
		const chargeCost = (route) => {
			...
			if (projected !== void 0) cumulativeCost += projected.cost;
		};
	...
	* rewritten config when a switch occurs, or `undefined` to send as-is. */
	async function preemptiveQuotaCheck(resolved, agent, agentState, state, turn, step, signal, userSwitched) {
		...
		if (trip.below || costTrip) {
			...
			while (true) {
				...
				if ((await probeValidRoute(candidate.route, signal)).ok) {
					result = candidate;
					break;
				}
		...
		if (quota !== void 0 && quotaCheck !== void 0 && quotaCheck.remaining !== void 0 && (quota.warnAbsolute !== void 0 || quota.warnRatio !== void 0)) {
			const forecastSteps = quota.forecastSteps ?? 1;
			const burn = projected !== void 0 ? projected.cost * forecastSteps : 0;
			const projectedRemaining = quotaCheck.remaining - burn;
			const absoluteTrip = quota.warnAbsolute !== void 0 && projectedRemaining < quota.warnAbsolute;
			const ratioTrip = quota.warnRatio !== void 0 && quotaCheck.total !== void 0 && quotaCheck.total > 0 && projectedRemaining / quotaCheck.total < quota.warnRatio;
			...
			if (absoluteTrip || ratioTrip) {
				if (agentState.forecastWarnedRoute !== route) {
					...
					agent.session.append("llm/quota-warning", {
						...
						...quotaCheck.total === void 0 ? {} : { total: quotaCheck.total },
						...absoluteTrip && quota.warnAbsolute !== void 0 ? {
							threshold: quota.warnAbsolute,
							thresholdKind: "absolute"
						} : quota.warnRatio !== void 0 ? {
							threshold: quota.warnRatio,
							thresholdKind: "ratio"
						} : {},
						...projected !== void 0 ? {
							estimatedCost: projected.cost,
			...
			} else agentState.forecastWarnedRoute = void 0;
		}
	...
	ctx.on("agent/request-error", async (payload, next) => {
		...
		if (costCap !== void 0 && cumulativeCost >= costCap) {
			if (eligible) agent.session.append("llm/quota-warning", {
				turn,
				step,
	...
	ctx.on("internal/dispatch", (_mode, eventName, args) => {
		...
		if (usage !== void 0 && typeof usage.outputTokens === "number" && usage.outputTokens > 0) {
			const samples = agentState.outputSamples.get(source.provider);
			if (samples === void 0) agentState.outputSamples.set(source.provider, [usage.outputTokens]);
			else {
