...
The plugin promotes a switched route to `healthyRoute` only after it completes a model message successfully — observed via an `internal/dispatch` listener for `session/event` of type `assistant/message`. Once healthy, subsequent requests are redirected to the healthy route **unless** the user switches models (detected as a fresh primary that differs from the previous step's primary — the plugin never rewrites `primaryRoute` to a fallback, so a change there is the user's own selection).
