...
function validateExhausted(
  history: readonly SessionEvent[],
  event: SessionEvent<'llm/fallback-exhausted'>,
  fail: InvariantFailure,
): void {
  ...
  if (step !== stepBoundary.data.step || turn !== stepBoundary.data.turn) {
    fail(`llm/fallback-exhausted names turn ${turn}/step ${step}, but the open step is ${stepBoundary.data.turn}/${stepBoundary.data.step}`)
  }
}
