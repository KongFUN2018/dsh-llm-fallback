...
- **Pre-switch availability probe** — with `probe.enabled` set, each chosen fallback candidate is verified with a minimal real request (1 token by default) before the switch is issued: a candidate that fails the probe is banned for the session and the chain advances, so an unusable route never kills a turn. The rejected candidate is recorded as an `llm/fallback` event with reason `probe-failed` (the switch never completed).
...
The package ships a browser companion (declared through its `dsh.client` field) that the DSH web shell loads automatically whenever the node half is loaded. It registers three Conversation Definitions plus three keyed chat renderers, so the durable events surface where they happened:
...
- **`llm/fallback-exhausted`** renders one row when the chain is exhausted — `⛔ Fallback chain exhausted: b/m2 failed on request 3 (reason SERVER) — no route left`.
...
Register the plugin in your host config (`cordis.yml`) — the snippet below shows the plugin list section: the first `- name:` entry is your upstream provider plugin, the second registers this plugin alongside it.
...
`probe` (off by default) verifies each chosen fallback candidate with a minimal real request before the switch is issued — directory presence is not usability, and a route can be listed yet have no quota or a broken adapter. `maxTokens` (default 1) and `timeoutMs` (default 6000) keep the probe cheap; `prompt` customizes the no-op ping. A candidate that fails the probe is banned for the session, the chain advances to the next candidate, and the rejection is recorded as an `llm/fallback` event with reason `probe-failed`. Enable it in deployments where an unusable candidate must not kill a turn (a real-host `UNKNOWN_MODEL` is exactly this class of failure).
...
- `llm/fallback` — recorded immediately before switching: `{ turn, step, fromProvider, fromModel, toProvider, toModel, code, remaining, mode?, score?, reason? }`. `remaining` counts fallback chain entries at or after the selected route, not guaranteed viable candidates (under strategy/decision selection some may be banned or fail the floor), and it includes the selected route itself unless it was the very last entry. `mode` is the active strategy mode and `score` its cost-mode score for the selected route; `reason: 'probe-failed'` marks a candidate rejected by the availability probe — the switch never completed and the event documents the rejection.
